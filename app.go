...
func main() {
	...
	http.HandleFunc("/", checkAndReturnValue)
...
// checkAndReturnValue checks that url from the http.Request contains x and y coordinate and if this coordinate is
//  inside circle that is created by values form config file.
...
func checkAndReturnValue(w http.ResponseWriter, r *http.Request) {
...
func getValueByCoordinate(w http.ResponseWriter, coordinate []int, valuteResponse *utils.ValuteResponse) (string, bool) {
	var value string
	var err error
	if appConfig.Check(coordinate[0], coordinate[1]) {
		value, err = utils.GetDollarValue(valuteResponse)
	} else {
		value, err = utils.GetEuroValue(valuteResponse)
	}
	if err != nil {
		if err != nil {
...
func getValutes(w http.ResponseWriter) *utils.ValuteResponse {
	...
	valuteResponse := &utils.ValuteResponse{}
